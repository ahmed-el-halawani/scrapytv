...
var _tmdbMovie = require("./tmdbMovie");

var _model = require("./model");

var _methods = require("./methods");

var _egybest = require("./egybest/egybest.route");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const moviesRoute = (route, pssid) => {
  route.get("/movietest", async (req, res) => {
    const x = await (0, _index.getEgybestVideoLink)(req);
    res.send(x);
  });
  route.get("/getegybestMovieLinks", async (req, res) => {
    if (pssid == null) {
      (0, _index.repetedRequest)();
    }
    ...
    try {
      const result = await (0, _index.getEgybestVideoLink)(req);
  ...
  route.get("/getakwamMovieLinks", async (req, res) => {
    ...
    try {
      const result = await (0, _index.getAkwamVidelLink)(req);
      res.status(200).json(result);
    } catch (error) {
      res.status(404).json({
        error
  ...
  route.get("/SearchForMovieInAkwam", async (req, res) => {
    try {
      ...
      const resData = await (0, _index.akwamSearch)(names, releaseDate);
      res.json(resData);
    } catch (error) {
      res.status(404).json(error);
    }
  ...
  route.get("/getEgybestVideoLink2/:name", async (req, res) => {
    ...
    const url = (await (0, _index.getEgybestWatchLink)(req.params.name)).watch;

    if (pssid == null) {
      console.log(req.params.name);
      const f = await (0, _index.getpssid3)(url);
      console.log(f);
      ...
      res.status(200).json({
        first: f,
        playList: (await (0, _index.extractSourcesFromEgybestStream)(f.href)).data
      });
      return;
    }
    ...
    if (pssid != null) {
      try {
        ...
        res.status(200).json({
          pssid,
          href,
          playList: (await (0, _index.extractSourcesFromEgybestStream)(_constants.HOST + href)).data
        });
        return;
      } catch (error) {
        console.log("i am in catch"); // console.log(error);
  ...
  route.get("/movies", async (req, res) => {
    try {
      let movie = await (0, _index.movies)(req);
  ...
  route.get("/watch/:name", async (req, res) => {
    res.status(200).json({
      watch: (await (0, _index.getEgybestWatchLink)(req.params.name)).watch
    });
  });
  ...
  route.get("/moviesFromTmdb", async (req, res) => {
    try {
      ...
      var toDayMovie = (await (0, _index.prepareTreands)((_req$query$page = req.query.page) !== null && _req$query$page !== void 0 ? _req$query$page : "1")).page;
  ...
  route.get("/searhForMovieInTmdb", async (req, res) => {
    try {
      ...
      const names = (0, _index.getNamesFromReq)(req);
      const releaseDate = req.query["releasedate"];
      const resData = await (0, _tmdbMovie.tmdbSearch)(names, releaseDate);
      res.json(resData);
    } catch (error) {
      res.status(404).json(error);
    }
  ...
  route.get("/tmdb/trending", async (req, res) => {
    try {
      const resData = await (0, _methods.TmdbTrending)({
  ...
  route.get("/moviesFromTmdb/ui", async (req, res) => {
    try {
      var _req$query$page2;

      var toDayMovie = (await (0, _index.prepareTreands)((_req$query$page2 = req.query.page) !== null && _req$query$page2 !== void 0 ? _req$query$page2 : "1")).page;
  ...
  route.get("/movies/ui", async (req, res) => {
    try {
      let movie = await (0, _index.movies)({
        req
      });
      res.status(200).send(`
      <!DOCTYPE html>
      ...
      <body>
      <h1>ui data</h1>
        ${movie.data.map(e => e.img != undefined ? `<a href=${_constants.HOST + /movie/ + e.u} target="_blank" class="movie" >
  ...
  (0, _egybest.egybestMoviesRoute)(route, pssid);
};
