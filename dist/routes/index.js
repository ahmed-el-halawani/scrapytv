...
route.get("/movies", async (req, res) => {
  ...
  }); //   const { data } = await fetch(`${HOST}/trending?page=${page}`);
...
route.get("/series/:u", async (req, res) => {
  const {
    data
  } = await _axios.default.get(`${_constants.HOST}/series/${req.params.u}`);
...
route.get("/series", async (req, res) => {
  ...
  const {
    data
  } = await _axios.default.get(`${_constants.HOST}/tv?page=${page}`);
  const result = getObjectFromHtml((0, _cheerio.default)("div #movies", data).children("*"));
  res.status(200).json(result);
});
